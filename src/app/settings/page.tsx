...
export default function SettingsPage() {
  ...
  // 按钮没反应。直接跳 /login 给一个清晰的终点：退出登录是一个明确的动作，
  // 不该把人送回 /ledger 的访客视图（技术上访客也能看本地账本，但那样"登出"
  // 这个动作本身没有任何可见结果，用户会怀疑是不是真的登出了）。根路径 /
  // 现在是按登录态分流的路由页（见 app/page.tsx），也不能直接跳那儿——刚
  // 登出的这一刻 useSession 缓存可能还没来得及更新，跳 / 有极小概率被当成
  // "已登录"弹回 /ledger（殊途同归但多一次无意义的跳转）。用 window.location
  // 而不是 next/navigation 的 useRouter——后者需要 App Router context，组件
  // 测试里没有挂载真实路由树会直接抛 invariant；login/page.tsx 的既有跳转
  // 就是这个写法，这里保持一致。
      ...
      <div className="flex flex-col gap-4">
        ...
        {usage && (
          <SettingsCard>
            ...
            <p className="mt-2 text-sm leading-relaxed text-muted">{t('settingsStorageDescription')}</p>
            <div
              role="progressbar"
        ...
        <SettingsCard>
          ...
          <p className="mt-2 text-sm leading-relaxed text-muted">{t('settingsDataDescription')}</p>
        </SettingsCard>
