...
describe('periodRange', () => {
  it('返回当月第一天到下月第一天（不含），按指定时区', () => {
    ...
    expect(periodRange(referenceDate, 'Australia/Sydney')).toEqual({
      start: '2026-09-01',
      end: '2026-10-01',
    });
  });
  ...
  it('12 月要跨年到下一年 1 月', () => {
    ...
    expect(periodRange(referenceDate, 'UTC')).toEqual({
      start: '2026-12-01',
      end: '2027-01-01',
    });
  });
...
describe('computeStats', () => {
  ...
  it('周期内没有账目时返回空数组，不报错', () => {
    expect(computeStats([], referenceDate, 'UTC')).toEqual([]);
  });
});
